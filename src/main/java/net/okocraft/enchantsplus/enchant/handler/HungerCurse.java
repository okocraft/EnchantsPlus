...
import net.okocraft.enchantsplus.config.Config.HungerCurseConfig;
...
public class HungerCurse extends TickDependentEnchantHandler<HungerCurseConfig> {

    public HungerCurse(EnchantsPlus plugin) {
        super(plugin, plugin.getMainConfig().getHungerCurseConfig());
    }
    ...
    public void handle(PlayerTickEvent event) {
        ...
        for (EquipmentSlot slot : EquipmentSlot.values()) {
            ...
            totalLevel += item.getCustomEnchantLevel(EnchantPlus.HUNGER_CURSE);
        }

        if (random.nextDouble() > totalLevel * config.getHungerRatePerLevel()) {
            return;
        }
